...
public class panelVendedor extends JPanel {
	//datos globales
	public cVendedor list=new cVendedor();  //  @jve:decl-index=0:
    ...
    public Vendedor leer()
    {
        ...
        if(form_validado()){
          ...
          //cedula_v, nombres_v, apellidos_v, telefono_v, ciudad_v.
            ...
            ob.telefono=txtDireccion.getText();
            ob.Ciudad=txtPadre.getText();
    ...
    public boolean form_validado(){
        ...
        if(!Validaciones.esTelefono(txtDireccion)){
            ok=false;
            men += ", Dirección";
        }

        if(!Validaciones.esLetras(txtPadre)){
            ok=false;
            men += ", Dirección";
        }
    ...
    public void ver_registro(int pos)
    {
        ...
        {
            Vendedor ob=list.getVendedor(pos);
            txtCedula.setText(ob.cedula);
            txtApellidos.setText(ob.apellidos);
            txtNombres.setText(ob.Nombres);
            txtDireccion.setText(ob.telefono);
            txtPadre.setText(ob.Ciudad);
    ...
    /*
     * Guardar Vendedor, cuando es nuevo o se modifica un existente
     */
    public void guardar(){
    	Vendedor ob=leer();
		try{
			if(ob!=null){
				if (ced.equals("")){//guardar un nuevo Vendedor
					list.nuevo(ob);
				}
				else{//guardar datos de Vendedor editado
    ...
    {
        try{
            cVendedor p=list.buscar_cedula(txtDato.getText()); //busca por cedula
            if(p.Count()==0)p=list.buscar_apellidos(txtDato.getText()); //buscar por apellido
	 ...
	 * @throws IOException
	 */
	public panelVendedor() throws IOException {
		super();
		initialize();
		list.leer();
		//formato de la tabla
	...
	private void initialize() {
		...
		lbTitulo.setText("Gestión de Vendedors");
	...
	private JPanel getPaneldatos() {
		if (paneldatos == null) {
			lbpadre = new JLabel();
			lbpadre.setBounds(new Rectangle(13, 144, 84, 25));
			lbpadre.setFont(new Font("Dialog", Font.PLAIN, 12));
			lbpadre.setText("Ciudad");
			lbdireccion = new JLabel();
			lbdireccion.setBounds(new Rectangle(14, 114, 84, 25));
			lbdireccion.setFont(new Font("Dialog", Font.PLAIN, 12));
			lbdireccion.setText("Telefono");
			lbbombre = new JLabel();
			...
			paneldatos.setBounds(new Rectangle(15, 53, 546, 182));
			paneldatos.setBorder(BorderFactory.createTitledBorder(null, "Datos de Vendedors", TitledBorder.DEFAULT_JUSTIFICATION, TitledBorder.DEFAULT_POSITION, new Font("Dialog", Font.BOLD, 12), new Color(51, 51, 51)));
	...
	private JTextField getTxtDireccion() {
		if (txtDireccion == null) {
			...
			txtDireccion.setLocation(new Point(177, 120));
			txtDireccion.setSize(new Dimension(330, 25));
			txtDireccion.addKeyListener(new java.awt.event.KeyAdapter() {
				public void keyReleased(java.awt.event.KeyEvent e) {
					if(Validaciones.esTelefono(txtDireccion)) Validaciones.pinta_text(txtDireccion);
				}
			});
		}
		return txtDireccion;
	...
	private JTextField getTxtPadre() {
		if (txtPadre == null) {
			...
			txtPadre.setLocation(new Point(178, 150));
	...
	private JPanel getPanelBuscar() {
		if (panelBuscar == null) {
			...
			panelBuscar.setLocation(new Point(15, 240));
	...
	private JButton getBtListar() {
		if (btListar == null) {
			...
			btListar.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					tabla.setModel(panelVendedor.this.list.getTabla());
